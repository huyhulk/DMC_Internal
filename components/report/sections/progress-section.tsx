...
const STATUS_COLOR: Record<OrderStatus['status'], string> = {
  ...
  completed_late: 'bg-[#ff9500]/10 text-[#b37700] border-[#ff9500]/20',
  in_progress:    'bg-[#3b82f6]/10 text-[#3b5bdb] border-[#3b5bdb]/20',
  due_soon:       'bg-[#ffcc00]/10 text-[#8a6d00] border-[#ffcc00]/20',
...
export function ProgressDetail({ orders, summary }: {
  ...
  const completedOnTimePct = summary.total > 0 ? (summary.completedOnTime / summary.total) * 100 : 0
  const completedLatePct = summary.total > 0 ? (summary.completedLate / summary.total) * 100 : 0
  ...
  const kpiItems: { label: string; value: number; color: string; activeColor: string; filter: OrderStatusCode | null }[] = [
    ...
    { label: 'HT đúng', value: summary.completedOnTime, color: 'text-[#2f9e44]', activeColor: 'bg-[#34c759]/15 border-[#2f9e44]/30', filter: 'completed' },
    { label: 'HT trễ', value: summary.completedLate, color: 'text-[#b37700]', activeColor: 'bg-[#ff9500]/10 border-[#ff9500]/30', filter: 'completed_late' },
    { label: 'Trễ hạn', value: summary.overdue, color: 'text-[#ff3b30]', activeColor: 'bg-[#ff3b30]/10 border-[#ff3b30]/30', filter: 'overdue' },
    { label: 'Sắp hạn', value: summary.dueSoon, color: 'text-[#8a6d00]', activeColor: 'bg-[#ffcc00]/10 border-[#ffcc00]/30', filter: 'due_soon' },
  ]
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-4">
        <div className="flex flex-1 h-3 bg-[#e5e5ea] rounded-full overflow-hidden">
          <div
            className="h-full bg-[#34c759] transition-all"
            style={{ width: `${completedOnTimePct.toFixed(1)}%` }}
          />
          <div
            className="h-full bg-[#ff9500] transition-all"
            style={{ width: `${completedLatePct.toFixed(1)}%` }}
          />
        </div>
...
export function ProgressComparison({ summaries }: { summaries: ProgressSummary[] }) {
  const chartData = summaries.map((s) => ({
    name: s.workshop,
    'HT đúng (%)': s.total > 0 ? Math.round((s.completedOnTime / s.total) * 1000) / 10 : 0,
    'HT trễ (%)': s.total > 0 ? Math.round((s.completedLate / s.total) * 1000) / 10 : 0,
  ...
  return (
    <div className="space-y-4">
      ...
      <table className="w-full text-[12px]">
        <thead className="bg-[#f2f2f7]">
          <tr>
            {['Xưởng', 'Tổng LSX', 'HT đúng', 'HT trễ', 'Trễ hạn', 'Sắp hạn', 'Tiến độ'].map((h) => (
        ...
        <tbody>
          {summaries.map((s) => (
            <tr key={s.workshop} className="border-t border-[#d2d2d7]/40 hover:bg-[#f2f2f7]/60">
              ...
              <td className="px-3 py-2 text-center text-[#2f9e44] font-medium">{s.completedOnTime}</td>
              <td className="px-3 py-2 text-center text-[#b37700] font-medium">{s.completedLate}</td>
              <td className="px-3 py-2 text-center text-[#ff3b30] font-medium">{s.overdue}</td>
              <td className="px-3 py-2 text-center text-[#8a6d00] font-medium">{s.dueSoon}</td>
