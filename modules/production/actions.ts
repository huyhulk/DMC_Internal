...
import { getProductionOrderVisibilityPolicy, shouldIncludeProductionOrderRow } from '@/modules/production/order-policy'
import {
...
import { requireTabEdit, requireTabView } from '@/modules/permissions/server'
...
export async function getInitData(
  ...
  try {
    ...
    const [norms, materials, dataRes, productionRes, visibilityPolicy] = await Promise.all([
      getCachedNorms(),
      getCachedMaterials(),
      supabase
        .from('data')
      ...
      getProductionOrderVisibilityPolicy(),
    ])
    ...
    const orders: Order[] = ((dataRes.data ?? []) as DataRow[])
      .filter((row) => shouldIncludeProductionOrderRow({
        rawWorkshop: row.WORKSHOP,
        role,
        userWorkspaces,
        validWorkshopCodes: validWorkshops,
        hasNormData,
