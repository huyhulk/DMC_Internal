-- ============================================================
-- 051: KHÔI PHỤC nhánh "Công trình" trong check_production_insert_permission.
--   048 & 049 là CREATE OR REPLACE toàn hàm nhưng QUÊN chép lại nhánh công trình
--   (đã thêm ở 042) → nhập LSX công trình bị "new row violates RLS policy for Production".
--   Bản này = hàm của 049 (DMC1/DMC3-PN/DMC4/DMC5) + nhánh CONG_TRINH (Hoạt động thi công).
--   Scoping công trình dùng RETURN tổng quát cuối hàm (workspace chứa 'CONG_TRINH' | ADMIN | ALL).
-- ============================================================
...
BEGIN
  ...
  ELSIF v_workshop ILIKE 'Phân xưởng 3%' OR upper(v_workshop) = 'DMC3' THEN
    ...
    IF coalesce(v_description, '') ILIKE '%pn%' OR coalesce(v_description, '') ILIKE '%panel%' THEN
      v_entry_workshop := 'DMC3-PN';
    ELSIF coalesce(v_description, '') ILIKE '%phụ kiện%'
