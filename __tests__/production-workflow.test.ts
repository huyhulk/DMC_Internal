...
describe('production workflow helpers', () => {
  ...
  it('scopes construction site workshop separately from DMC1 workspaces', () => {
    expect(isProductionEntryWorkspaceAllowed(CONSTRUCTION_WORKSHOP_CODE, 'USER', [CONSTRUCTION_WORKSHOP_CODE], CONSTRUCTION_WORKSHOP_CODE)).toBe(true)
    expect(isProductionEntryWorkspaceAllowed('Hoạt động thi công tại công trình', 'USER', [CONSTRUCTION_WORKSHOP_CODE], CONSTRUCTION_WORKSHOP_CODE)).toBe(true)
    expect(isProductionEntryWorkspaceAllowed(CONSTRUCTION_WORKSHOP_CODE, 'USER', ['DMC1'], 'DMC1')).toBe(false)
  })
